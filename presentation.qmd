...
# What we are doing, in VAR terms
$$
\begin{aligned}
...
$$
\begin{aligned}
\text{or } \begin{bmatrix} 1 & -\beta \\ \color{red}{0} & 1\end{bmatrix} \begin{bmatrix} y_{it} \\ d_{it} \end{bmatrix} = \begin{bmatrix} \alpha_{11} & \alpha_{12} \\ \alpha_{21} & \alpha_{22} \end{bmatrix} \begin{bmatrix} y_{i, t-1} \\ d_{i,t-1}\end{bmatrix} + \begin{bmatrix} \color{red}{0} \\ \delta \end{bmatrix} z_i + \begin{bmatrix}\epsilon_{y,it} \\ \epsilon_{d,it}\end{bmatrix}.
\end{aligned}
$$
...
- We are simply conducting a Cholesky decomposition. 

\pause

## What most IV papers are doing, in VAR terms
$$
\begin{aligned}
...
- There is one development econ paper which talks about this (Casey and Klemp 2021), but their solution is questionable. 
  ...
  - The lag length is arbitrary. And even if it works, their method can't recovers only a "long-run" parameter, not the instantaneous parameter that is of policy interest. \pause
...
2. The causal literature has been misspecifying a lot of IV papers. \pause
    - Part of the causal revolution-revolution: TWFE DID estimator (Callaway & Sant'Anna 2020), issues with geographic IVs (Mellon 2022).
